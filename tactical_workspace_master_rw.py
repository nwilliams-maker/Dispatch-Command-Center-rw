...
.stTabs [data-baseweb="tab"] {{ border-radius: 30px !important; margin: 0 5px !important; font-weight: 800 !important; padding: 8px 25px !important; border: 2px solid transparent !important; }}
h1, h2, h3 {{ font-weight: 800 !important; text-align: center !important; }}
...
def fetch_sent_records_from_sheet():
    try:
        ...
        sent_dict = {}
        ghost_routes = {p: [] for p in list(POD_CONFIGS.keys()) + ["UNKNOWN"]}
                ...
                if 'json payload' in df.columns:
                    for _, row in df.iterrows():
                        try:
                            ...
                            c_name = str(row.get('contractor', 'Unknown'))
...
def move_to_dispatch(cluster_hash, ic_name, pod_name, action_label="Revoked", check_onfleet=False):
    ...
    for c in cls:
        ...
        if hashlib.md5("".join(sorted(ids)).encode()).hexdigest() == cluster_hash:
            st.session_state[f"reverted_{cluster_hash}"] = True
            st.toast(f"✅ {action_label} route pulled back!")
            st.rerun()

# --- CORE ENGINE ---
def process_pod(pod_name, master_bar=None, pod_idx=0, total_pods=1):
    config = POD_CONFIGS[pod_name]
    if master_bar: master_bar.progress(pod_idx/total_pods, text=f"[{pod_name}] Fetching Onfleet...")
    
    try:
        # Fetch fresh DB status
        ...
        for t in all_tasks_raw:
            ...
            tt_val = str(t.get('taskType', '')).lower()
            for m in (t.get('metadata') or []):
                m_name = str(m.get('name', '')).lower()
                m_val = str(m.get('value', '')).lower()
                if "task type" in m_name or any(x in m_val for x in ["digital", "ins", "offline", "kiosk"]):
                    tt_val += f" {m_val}"
            ...
            pool.append({
                ...
                "task_type": tt_val, "db_status": db_entry.get('status', 'ready').lower(), "wo": db_entry.get('wo', 'none')
            })

        clusters = []
        while pool:
            anc = pool.pop(0)
            anc_is_dig = any(x in anc['task_type'] for x in ["digital", "ins", "offline"])
            rad = 25 if anc_is_dig else 50
            
            group = [anc]
            rem = []
            for t in pool:
                t_is_dig = any(x in t['task_type'] for x in ["digital", "ins", "offline"])
                if anc_is_dig == t_is_dig and haversine(anc['lat'], anc['lon'], t['lat'], t['lon']) <= rad:
                    if anc['db_status'] in ['sent', 'accepted'] and t['wo'] != anc['wo']:
                        rem.append(t)
                    else: group.append(t)
                else: rem.append(t)
...
def render_dispatch(i, cluster, pod_name, is_sent=False, is_declined=False):
    ids = [str(t['id']).strip() for t in cluster['data']]
    c_hash = hashlib.md5("".join(sorted(ids)).encode()).hexdigest()
    
    stop_metrics = {}
    for t in cluster['data']:
        addr = t['full']
        if addr not in stop_metrics: stop_metrics[addr] = {'t': 0, 'dig': 0, 'ins': 0, 'new': 0}
        stop_metrics[addr]['t'] += 1
        tt = t['task_type']
        if any(x in tt for x in ["digital", "ins", "offline", "service"]): stop_metrics[addr]['dig'] += 1
        elif "install" in tt: stop_metrics[addr]['ins'] += 1
        else: stop_metrics[addr]['new'] += 1

    for addr, m in stop_metrics.items():
        pills = []
        if m['dig'] > 0: pills.append(f"🔌 {m['dig']} Digital")
        if m['ins'] > 0: pills.append(f"🛠️ {m['ins']} Kiosk")
        if m['new'] > 0: pills.append(f"🆕 {m['new']} New Ad")
        st.markdown(f"**{addr}** <small>— {' | '.join(pills)}</small>", unsafe_allow_html=True)
    ...
    if not ic_df.empty:
        ...
        if ic_opts:
            ...
            if st.button("🚀 Send to Portal", key=f"btn_{c_hash}", use_container_width=True):
                st.success("Link logic triggered.")

def run_pod_tab(pod_name):
    if f"clusters_{pod_name}" not in st.session_state:
        if st.button(f"Initialize {pod_name}", use_container_width=True): 
    ...
    for c in cls:
        ...
        if match and not is_rev:
            ...
            c['contractor_name'] = match['name']
            c['route_ts'] = match['time']
    ...
    with col_left:
        ...
        st.markdown(f"<h3 style='color:{TB_PURPLE}'>🚀 Dispatch</h3>", unsafe_allow_html=True)
        t_r, t_f = st.tabs(["Ready", "Flagged"])
        with t_r:
    ...
    with col_right:
        st.markdown(f"<h3 style='color:{TB_GREEN}'>⏳ Awaiting</h3>", unsafe_allow_html=True)
        ts, ta, td, tf = st.tabs(["Sent", "Accepted", "Declined", "Finalized"])
            ...
            for i, c in enumerate(accepted):
                ...
                with exp:
                    with st.expander(f"✅ {c.get('wo','Route')} | {c['city']}{c['h_icons']}"):
                        ...
                        if st.checkbox("3. Pack List Uploaded?", key=f"s3_{c_hash}", disabled=not s2):
                            finalize_route_handler(c_hash)
                            st.rerun()
        ...
        with td:
            for i, c in enumerate(declined):
                ...
                with btn:
                    if st.button("↩️ Re-Route", key=f"rr_{c_hash}"): move_to_dispatch(c_hash, "", pod_name)
        ...
        with tf:
            for i, c in enumerate(finalized):
                ...
                c_hash = hashlib.md5("".join(sorted(ids)).encode()).hexdigest()
                with st.expander(f"🏁 {c['contractor_name']} | {c['city']}{c['h_icons']}"):
                    st.info("Route Finalized.")

# --- MAIN APP ---
col_t, col_r = st.columns([8, 2])
with col_t: st.markdown(f"<h1 style='color:{TB_PURPLE}'>Dispatch Command Center</h1>", unsafe_allow_html=True)
with col_r:
    ...
    for i, (pod, config) in enumerate(POD_CONFIGS.items()):
        with cols[i]:
            ...
            if has_data:
                ...
                st.metric(pod, f"{len(accepted)} Accepted", f"{len(sent)} Pending")
